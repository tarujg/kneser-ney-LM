...
public class KneserNeyLm implements NgramLanguageModel
{
    ...
    */
    ...
    public double getNgramLogProbability(int[] ngram, int from, int to) {
        return Math.max(Math.log(getNgramProbability(ngram, from, to)),-100);
    }
    ...
    public double getNgramProbability(int[] ngram, int from, int to) {
        ...
        if (to - from == 3){
            ...
            return (bigramNormalizer.length > ngram[from] && bigramNormalizer[ngram[from]] > 0)?
                    //P_continuation
                    (Math.max(bigramFertility.get(bigramIndex) - d, 0)/bigramNormalizer[ngram[from]]) +
                        //BackOff - alpha*P_backoff
                        ((double)w1x[ngram[from]]*d/ bigramNormalizer[ngram[from]])*getNgramProbability(ngram, from+1, to)
                        : getNgramProbability(ngram,from+1,to);
        }
